export default function BenefitCard({ benefit, onSelect }) {
  return (
    <div 
    onClick={() => onSelect({title, imagen_url, description, category, text_discount, discount})}
    className="mx-auto bg-white rounded-2xl shadow-lg overflow-hidden w-full max-w-sm md:max-w-xl hover:shadow-2xl hover:transform hover:scale-105 duration-300 transition-all cursor-pointer">
      ...
      <div className="p-4">
        ...
        <h2 className="font-semibold text-start text-base lg:text-lg text-gray-700">{title}</h2>
        <p className="mt-2 text-lg lg:text-xl font-bold text-gray-800">{text_discount ? text_discount : discount + " % off"}</p>
